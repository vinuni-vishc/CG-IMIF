...
class Appr(Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images_rgb, histo_infos, targets in trn_loader:
            images_rgb, histo_infos, targets = images_rgb.to(self.device), histo_infos.to(self.device), targets.to(self.device)
            # Forward current model
            outputs, features = self.model(images_rgb, histo_infos, return_features=True)
            # Forward previous model
            ref_outputs = None
            ref_features = None
            
            if t > 0:
                ref_outputs, ref_features = self.ref_model(images_rgb, histo_infos, return_features=True)
