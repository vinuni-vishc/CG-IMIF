...
from .incremental_learning_3stream import Inc_Learning_Appr
from datasets.exemplars_dataset_3stream import ExemplarsDataset
...
class Appr(Inc_Learning_Appr):
        ...
        if t == 0:
            ...
            if self.model.model.__class__.__name__ == 'ResNet':
                ...
                self.model.model.layer3[-1] = BasicBlockNoRelu(old_block.conv1, old_block.bn1, old_block.relu,
                                                               old_block.conv2, old_block.bn2, old_block.conv3, old_block.bn3,
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images_rgb, images_edge, histo_infos, targets in trn_loader:
            images_rgb, images_edge, histo_infos, targets = images_rgb.to(self.device), images_edge.to(self.device), histo_infos.to(self.device), targets.to(self.device)
            # Forward current model
            outputs, features = self.model(images_rgb, images_edge, histo_infos, return_features=True)
            # Forward previous model
            ref_outputs = None
            ref_features = None
            
            if t > 0:
                ref_outputs, ref_features = self.ref_model(images_rgb, images_edge, histo_infos, return_features=True)
            ...
            if self.margin_ranking:
                ...
                if hard_num > 0:
                    ...
                    loss_mr = nn.MarginRankingLoss(margin=self.dist)(gt_scores.view(-1, 1),
                                                                     ...
                                                                     torch.ones(hard_num * self.K).unsqueeze(1).to(self.device))
