...
class Appr(Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images_rgb, images_edge, targets in trn_loader:
            images_rgb, images_edge, targets = images_rgb.to(self.device), images_edge.to(self.device), targets.to(self.device)
            # Forward current model
            outputs, features = self.model(images_rgb, images_edge, return_features=True)
            # Forward previous model
            ref_outputs = None
            ref_features = None
            
            if t > 0:
                ref_outputs, ref_features = self.ref_model(images_rgb, images_edge, return_features=True)
