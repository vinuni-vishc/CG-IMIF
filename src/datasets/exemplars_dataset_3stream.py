...
from datasets.memory_dataset_3stream import MemoryDataset
...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, transform, class_indices,
        ...
        super().__init__({'x_rgb': [], 'x_edge': [], 'x_histo':[], 'y': []}, transform, class_indices=class_indices)
        ...
        selector_cls = getattr(importlib.import_module(name='datasets.exemplars_selection_3stream'), cls_name)
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            self.images_rgb, self.images_edge, self.histo_info, self.labels = self.exemplars_selector(model, trn_loader, selection_transform)
