...
class MemoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # x_rgb = Image.fromarray(self.images_rgb[index])
        # x_edge = Image.fromarray(self.images_edge[index])

        # x_rgb = self.transform['rgb'](x_rgb)
        # x_edge = self.transform['edge'](x_edge)

        
        x_rgb = self.images_rgb[index]
        x_edge = self.images_edge[index]
        x_histo = self.histo_info[index]

        y = self.labels[index]
        return x_rgb, x_edge, x_histo, y
