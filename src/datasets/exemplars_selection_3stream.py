...
class ExemplarsSelector:
    ...
    def __call__(self, model: LLL_Net, trn_loader: DataLoader, transform):
        ...
        with override_dataset_transform(trn_loader.dataset, transform) as ds_for_raw:
            x_rgb, x_edge, x_his, y = zip(*(ds_for_raw[idx] for idx in selected_indices))

        
        clock1 = time.time()
        print('| Selected {:d} train exemplars, time={:5.1f}s'.format(len(x_rgb), clock1 - clock0))
        return x_rgb, x_edge, x_his, y
